...
class HasPaperTrailModelTest < ActiveSupport::TestCase
  ...
  context 'A new model instance which uses a custom Version class with no base version table' do
    setup do
      drop_schema_table :paper_trail_versions
      @post = Post.new
    end
  ...
  context 'An existing model instance which uses a custom Version class with no base version table' do
    setup do
      drop_schema_table :paper_trail_versions
      @post = Post.create
    end    
